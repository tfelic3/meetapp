...
const credentials = {
	...
	redirect_uris: ["https://tfelic3.github.io/meetapp"],
	javascript_origins: ["https://tfelic3.github.io", 'http://localhost:3000'],
};
...
module.exports.getAccessToken = async (event) => {
	// new oAuth2Client
	const oAuth2ClientNew = new google.auth.OAuth2(
		client_id,
		client_secret,
		redirect_uris[0]
	);
...
module.exports.getCalendarEvents = async (event) =>{
	...
	.then((response) => {
			...
			return {
				...
				body: JSON.stringify({ event: results.data.items }),
			};
		})
		.catch((err) => {
			// Handle error
